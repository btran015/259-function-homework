...
limit_replace <- function(runif(x = NA)) {
  b <- (x < 2)
  c <- (x > 8)
return(y_replace)}
...
plus_minus_SD <- function(x, num_of_SDs) {
  sd(x)
  mean(x)
}
plus_minus_SD(x, 1)
...
limit_replace <- function(x = NA, if NA = 1) {
  b <- (x < 2)
  c <- (x > 8)
  return(y_replace)}
...
limit_replace <- function(x = NA, stopifnot(class = numeric) {
  b <- (x < 2)
  c <- (x > 8)
  return(y_replace)}
...
#Add another stopifnot to enforce one more requirement
  ...
  limit_replace <- function(x = NA, stopifnot(class = as.numeric, as.vector) {
    b <- (x < 2)
    c <- (x > 8)
    return(y_replace)}
...
limit_replace(ds_diamonds, stim_name = "Diamonds")
plus_minus_SD(ds_diamonds, stim_name = "SD", save_file = T))
...
limit_replace(ds_diamonds, na.rm = T)
ds_trimmed <- as.tibble(y_replace)
